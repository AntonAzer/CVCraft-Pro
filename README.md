...
2. In `index.html`, set:
   ...
   ```
   to wherever the backend is running. Once the backend is deployed on Vercel,
   change this to its live URL, e.g. `https://cvcraft-backend.vercel.app/api`.
...
This is a static site (no build step), so Vercel's default "Static" or
"Other" framework preset works with no `vercel.json` needed — just import the
repo. Two separate Vercel projects (one per repo, backend and frontend) is
the right setup; just make sure:
- The frontend's `CVCRAFT_API_BASE_URL` points at the backend's Vercel URL.
- The backend's `CORS_ORIGIN` env var points at the frontend's Vercel URL.
